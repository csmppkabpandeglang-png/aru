    ...
    const loadData = async () => {
      try {
        ...
        const [
          ...
          leadService.getAll(),
          cardService.getAll(),
          financialPocketService.getAll(),
          contractService.getAll(),
          assetService.getAll(),
          clientFeedbackService.getAll(),
          notificationService.getAll(),
          socialMediaPostService.getAll(),
        ...
        // Calculate derived data (team project payments, reward entries, etc.)
        const teamPaymentsData: TeamProjectPayment[] = [];
        projectsData.forEach(project => {
          project.team.forEach((teamMember, index) => {
            teamPaymentsData.push({
              id: `tpp-${project.id}-${index}`,
              projectId: project.id,
              teamMemberName: teamMember.name,
              teamMemberId: teamMember.memberId,
              date: project.date,
              status: transactionsData.some(t => 
                t.projectId === project.id && 
                (t as any).teamMemberId === teamMember.memberId && 
                t.category === 'Gaji Freelancer'
              ) ? 'Paid' : 'Unpaid',
              fee: teamMember.fee,
        ...
        // Calculate reward ledger entries
        const rewardEntries: RewardLedgerEntry[] = transactionsData
          .filter(t => (t.category === 'Hadiah Freelancer' || t.category === 'Penarikan Hadiah Freelancer') && (t as any).teamMemberId)
          .map(t => ({
            id: `rle-${t.id}`,
            teamMemberId: (t as any).teamMemberId!,
            date: t.date,
        ...
        setRewardLedgerEntries(rewardEntries);

      } catch (error) {
        console.error('Failed to load data:', error);
        showNotification('Gagal memuat data dari database', 5000);
      } finally {
        setLoading(false);
      }
    };
